...
exports.init_cassandra = function(req, res){
	client.connect()
		.then(function () {
			const query = "CREATE KEYSPACE IF NOT EXISTS movies WITH replication =" +
			  "{'class': 'SimpleStrategy', 'replication_factor': '1' }";
			return client.execute(query);
		})
		.then(function () {
			const query = "CREATE TABLE IF NOT EXISTS movies.movieList" +
				" (id uuid, movie text, year text, genre text, rating text, comment text, PRIMARY KEY (id))";
			return client.execute(query);
		})
		.then(function () {
			return client.metadata.getTable('movies', 'movieList');
		})
		.then(function (table) {
